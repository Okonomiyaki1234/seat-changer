...
import { Shuffle, Save, Upload, Plus, X, Grid3x3, Users, FilePlus } from 'lucide-react';
...
export default function Home() {
  ...
  const loadTemplate = async () => {
    ...
    try {
      ...
      if (data) {
        ...
        toast.error('クラスコードが見つかりません。新規作成は「新規テンプレート作成」から行ってください');
      }
    } catch (error) {
  ...
  const saveTemplate = async () => {
    ...
    if (!currentTemplate) {
      toast.error('保存できません。新規作成は「新規テンプレート作成」から行ってください');
      return;
    }
  ...
  const createTemplate = async () => {
    ...
    try {
      const { data: existing } = await supabase
        .from('classroom_templates')
        .select('id')
        .eq('class_code', newClassCode.trim())
        .maybeSingle();

      if (existing) {
        toast.error('このクラスコードは既に存在します。既存テンプレートの読み込みから操作してください');
        return;
      }

      const templateData = {
        class_code: newClassCode.trim(),
        seat_layout: { rows: 6, cols: 6, disabled: [] },
        students: [],
        updated_at: new Date().toISOString(),
      };
      ...
      setCurrentTemplate(data);
      setClassCode(newClassCode.trim());
      setSeatLayout({ rows: 6, cols: 6, disabled: [] });
      setStudents([]);
      setAssignments([]);
      setNewClassCode('');
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <Card className="shadow-lg">
            ...
            <CardContent className="space-y-3">
              ...
              <Button
                ...
                variant={currentTemplate ? 'default' : 'secondary'}
              >
                <Save className="w-4 h-4" />
                {currentTemplate ? `「${currentTemplate.class_code}」を更新保存` : '（テンプレートを読み込んでください）'}
              </Button>
            </CardContent>
          </Card>
          ...
          <Card className="shadow-lg border-green-200">
            <CardHeader>
              ...
              <CardDescription>
                新しいクラスコードを入力してテンプレートを作成します。既存のコードは使用できません。
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex gap-2">
                <Input
                  ...
                  className="text-lg border-green-300 focus-visible:ring-green-400"
                />
                <Button
                  onClick={createTemplate}
                  disabled={isLoading || !newClassCode.trim()}
